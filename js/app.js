...
App.refreshFromSheets=async function(){
  ...
  try{
    const result=await GoogleSheetsService.synchronize(true);
...
App.init=async function(){
  ...
  const sync=await GoogleSheetsService.synchronize(false);
  App.rebuildDataIndexes();
  App.populateBaseSelectors();
  ...
  App.$('#cliente').onchange=App.fillClient;App.$('#anoTabela').onchange=()=>{App.updateMethodUI();App.recalcAll();};App.$('#addLinkBtn').onclick=()=>App.addAccessRow('link');App.$('#addInternetBtn').onclick=()=>App.addAccessRow('internet');App.$('#addTiBtn').onclick=()=>App.addTiRow('regular');App.$('#addTiAnnualBtn').onclick=()=>App.addTiRow('annual');App.$('#addInstallationBtn').onclick=()=>App.addInstallationRow();App.initTableToggles();App.$('#printBtn').onclick=()=>window.print();App.$('#saveBtn').onclick=App.save;App.$('#newBtn').onclick=App.reset;App.$('#syncBtn').onclick=App.refreshFromSheets;App.updateMethodUI();const saved=localStorage.getItem('orcamentoAtual');if(saved){try{App.load(JSON.parse(saved));}catch(e){console.error(e);localStorage.removeItem('orcamentoAtual');}}if(!document.querySelector('#installations tr')){App.addInstallationRow();App.addAccessRow('link');App.addAccessRow('internet');App.addTiRow('regular');App.addTiRow('annual');}};
